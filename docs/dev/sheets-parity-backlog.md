...
The menu roadmap lives in the Notion Tasks Tracker; this file is the list of Sheets features the
menus **do not** show, and why.
...
(ASNT-44 text wrapping, ASNT-79 merge/explode rows, ASNT-73 AI review). The gallery view
(ASNT-72) has shipped as **View ▸ Switch View ▸ Gallery**. Everything below has no task, so it has no menu entry either — a greyed-out
...
|---|---|
| Sort A→Z / Z→A, sort range | A sort descriptor persisted apart from the source data, and visible rows built by filtering then sorting row identities. The most obviously missing item; a cataloguer expects to sort by title. |
...
| Hide / resize row | Columns already resize. Hiding rows competes with filters, which are visible and reversible, while hidden-row state is easy to lose. Resizing rows is coupled to ASNT-44 (wrapped text needs variable row height). |
...
| Sharing, comments/discussions, offline, publish to web | Single-user desktop app over local files. Version history is the History panel's job: it keeps the change log and named versions inside the `.qrate` file. |
...
| Column stats, data connectors, refresh connected data | Spreadsheet-workbook concepts with no counterpart in a catalogue. Row grouping is covered by the archival hierarchy (nested rows, indent and outdent). |

## If any of these get built

Create the task in Notion first, fan it out to GitHub (see `CLAUDE.md`), then add the
