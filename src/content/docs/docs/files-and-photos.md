...
- **Your own pattern.** Configure a column as the `Filename` type to control which column
  and which matching rule qrate uses. See [Columns](/docs/columns).

If a linked file cannot be found, for example because the files folder moved or the file
was renamed, qrate reports it as a diagnostic. See [Diagnostics](/docs/diagnostics).
...
grid. See [The grid](/docs/grid#gallery-view).
