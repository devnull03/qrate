---
title: 'The grid'
description: 'edit cells, search, filter, and undo'
sidebar:
  order: 1
---
...
Right-click a row or column header for a menu to add, remove, rename, freeze, or reorder
it. A frozen row or column stays in view while you scroll.
...
Click the filter icon in a column header to open its filter. It lists every distinct value
...
Switch the grid to a gallery of thumbnails from the view menu. The gallery shows one tile
...
Right-click a cell and choose **Add note** to attach a free-text note to it. A cell with a
note shows a small corner marker. Notes are for your own reviewing use. They are not
exported.
