...
- **No speculative scaffolding.** Code "kept for later" outside the module tree, provider stubs full of `todo!()`, and builder methods only tests call are deletions, not investments — git history is the archive. **Two deliberate exceptions, do not flag or delete them:** the `ai` crate (`crates/ai/` — traits + Cohere/mock providers) is the planned home for AI review/embedding and stays despite its `todo!()` bodies; `crates/workspace/src/panels/log_viewer.rs` is the reusable line-coloring viewer that becomes the diagnostics content for `ProblemsPanel`, kept out of `mod.rs` until then.
